...
class Keymaker(object):
    ...
    def make_keys(self,files_dir):
        ...
        # 2. For each file, ask the user if they want to make a key for it
        consumer_token = self.consumer_token
        for myfile in files:

            full_file = re.sub('//','/',files_dir + '/' + myfile)
            ...
            if make_key == 'n':
                ...
                print 
                print "Sign in as the user to be associated with file "+full_file
                print 
                ...
                full_keys_file = re.sub('data','keys',full_file)
